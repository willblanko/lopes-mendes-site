...
// ─── Data (bios fictícias — substituir depois) ────────────────────────────────
      ...
      >
        ...
        <div style={{ padding: "36px 32px", display: "flex", flexDirection: "column", gap: "20px" }}>
          <div>
            ...
            {member.oab && (
              <p style={{ fontSize: "12px", color: "#aaa", margin: "4px 0 0" }}>{member.oab}</p>
            )}
          </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {member.areas.map((a) => (
                  <span key={a} style={{
                    ...
                    backgroundColor: "rgba(1,168,221,0.1)", color: "#004C90",
...
export default function EquipePage() {
  ...
  // pause auto when modal is open
  useEffect(() => {
    if (activeMember) stopAuto();
    else startAuto();
  }, [activeMember, startAuto, stopAuto]);

  function prev() {
    stopAuto(); setAnimated(true);
    setIndex((i) => i - 1);
    startAuto();
  }
  function next() {
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    const moved = Math.round(-(e.clientX - dragStartX.current) / STEP);
    setAnimated(false);
    ...
    <>
      <style>{`
        ...
        .carousel-btn {
          ...
          border: 2px solid #003567; background: white; color: #003567;
          cursor: pointer; display: flex; align-items: center; justify-content: center;
          font-size: 18px; transition: background 0.2s, color 0.2s; flex-shrink: 0;
        }
        ...
        .member-card {
          flex: 0 0 ${CARD_WIDTH}px; width: ${CARD_WIDTH}px;
          cursor: pointer;
        }
        .member-card-photo-wrap {
          position: relative; height: 340px; border-radius: 4px;
          overflow: hidden; background-color: #e0e4ec;
        }
        ...
        .member-card-overlay {
          ...
          padding: 48px 14px 14px; pointer-events: none;
        }
        .member-card-name {
        ...
        .member-card-hint {
          ...
          font-family: Lato, sans-serif; margin-top: 2px; display: block;
        }
        .member-card-info { padding: 10px 0 0; }
        .member-card-role {
          font-size: 11px; font-weight: 700; color: #01A8DD;
          ...
          >
            {tripled.map((m, i) => (
              <div
                ...
                role="button"
                tabIndex={0}
                aria-label={`Ver bio de ${m.name}`}
                onKeyDown={(e) => e.key === "Enter" && setActiveMember(m)}
              >
                <div className="member-card-photo-wrap">
                  <Image
                    ...
                    alt={m.name}
                    fill
                    className="member-card-photo-img"
                    sizes="260px"
                    draggable={false}
                  />
